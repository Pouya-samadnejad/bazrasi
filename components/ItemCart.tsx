import { SolarArrowLeftOutline } from "@/icons/SolarArrowLeftOutline";
...
function ItemCart({ link, title }) {
  return (
    <Link href={link} className="w-full sm:w-48 md:w-56 lg:w-64">
      <div className="group relative flex flex-col items-center justify-center h-48 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:shadow-lg hover:scale-[1.02]">
        ...
        <div className="absolute bottom-4 left-4">
          <SolarArrowLeftOutline className="w-6 h-6 text-gray-600 transition-transform duration-300 group-hover:-translate-x-1" />
        </div>
      </div>
    </Link>
  );
